...
describe('ACCEPTANCE - pureSave Product', function () {
    'use strict';
    ...
    var goToAccountApplicationScreens = function (user) {
        ...
        savingsPrescreeningWidget.actions.apply("pure-save");
        savingsPrescreeningWidget.actions.fraudCheckConsentCheckBoxClick("pure-save");
        savingsPrescreeningWidget.actions.fraudCheckConsentFormNextClick("pure-save");
    ...
    describe('When a customer that can apply for pureSave', function () {
        describe('PW-14271: Scenario 1, 2 and 3: User has all AML fields populated', function () {
            it('Should display the PureSave product tile', function () {
                goToSavingsAndInvestments(browser.params.credentials);
                expect(savingsPrescreeningWidget.getFraudCheckModal('pure-save').isDisplayed()).toBeFalsy();
            ...
            describe('PW 14272: Then clicks on Details button in the PureSave Product Tile', function () {
                it('Should show the PureSave product page', function () {
                    savingsAndInvestmentsOptionsPage.actions.pureSaveDetails();
                    expect(pureSaveProductPage.getTitle()).toEqual('Apply for Savings Account');
                    expect(pureSaveProductPage.baseActions.getCurrentUrl()).toContain('/apply/pure-save');
                    expect(pureSaveProductPage.getProductTitle()).toContain('PureSave');
                    expect(pureSaveProductPage.getPureSaveProductDescription()).toEqual('PureSave is an easy means to saving. Save what you can - and gain instant access to your money when you need it.');
                    expect(pureSaveProductPage.getContentHeaders()).toContain('Highlights');
                    expect(pureSaveProductPage.getContentHeaders()).toContain('How to qualify');
                    expect(pureSaveProductPage.getContentHeadersPanelTwo()).toContain('What it costs');
                    expect(pureSaveProductPage.getContentHeadersPanelTwo()).toContain('What you\'ll need');
                    expect(pureSaveProductPage.getPricingGuide()).toEqual('PureSave pricing (PDF)');
                    expect(pureSaveProductPage.getTitle()).toEqual('Apply for Savings Account');
                    expect(pureSaveProductPage.ApplyButtonVisible().isDisplayed()).toBeTruthy();
                    expect(pureSaveProductPage.backtoSavingInvestment().isDisplayed()).toBeTruthy();
                    expect(pureSaveProductPage.baseActions.getCurrentUrl()).toContain('/apply/pure-save');
                    expect(element(by.css('#pureSaveProductsTitle')).getText()).toEqual('Apply for Savings Account');
                });

                describe('Then clicks on the apply button', function () {
                    it('should display the fraud check consent form', function () {
                        savingsPrescreeningWidget.actions.apply("pure-save");
                        expect(savingsPrescreeningWidget.getFraudCheckModal("pure-save").isDisplayed()).toBeTruthy();
                        expect(savingsPrescreeningWidget.getFraudCheckModalNextButton("pure-save").isEnabled()).toBeFalsy();
                    });

                    describe('Then clicks on fraud check consent checkbox', function () {
                        it('should enable the next button', function () {
                            savingsPrescreeningWidget.actions.fraudCheckConsentCheckBoxClick("pure-save");
                            expect(savingsPrescreeningWidget.getFraudCheckModalNextButton("pure-save").isEnabled()).toBeTruthy();
                        });

                        describe('Then clicks on next button', function () {
                            it('Should create the PureSave flow and direct the user to the account application transfer page', function () {
                                savingsPrescreeningWidget.actions.fraudCheckConsentFormNextClick("pure-save");
                                expect(pureSaveProductPage.baseActions.getCurrentUrl()).toContain('/apply/pure-save/transfer');
                    ...
                    describe('Then clicks on cancel button', function () {
                        it('should close the fraud check consent form and show popup asking customer to agree to fraud check', function () {
                            goToSavingsAndInvestments(browser.params.credentials);
                            savingsPrescreeningWidget.actions.apply("pure-save");
                            savingsPrescreeningWidget.actions.fraudCheckConsentFormCancelClick("pure-save");
                            expect(savingsPrescreeningWidget.getFraudCheckModal("pure-save").isDisplayed()).toBeFalsy();
                        });
                    });
                });
            ...
            describe('Then Clicks on apply button for pureSave in the product tile', function () {
                it('should display the fraud check consent form', function () {
                    goToSavingsAndInvestments(browser.params.credentials);
                    savingsPrescreeningWidget.actions.apply("pure-save");
                    expect(savingsPrescreeningWidget.getFraudCheckModal("pure-save").isDisplayed()).toBeTruthy();
                ...
                describe('Then clicks on fraud check consent checkbox', function () {
                    ...
                    describe('Then clicks on next button', function () {
                        ...
                        describe('proceed to Account details screen validations', function () {
                            it('should display the Pure save account details screen and should through the amount exceeds error for low balance account', function () {
                            ...
                            it('should display the Pure save account details screen and should through the erro "Please enter the amount in a valid format" for the .3 digit cents', function () {
                                expect(transferPage.getPureSaveAccountDetails().getText()).toBe("PureSave Account Details");
                            ...
                            it('should display the Pure save account details screen and should through the minimum amount message for less 50 rands', function () {
                                expect(transferPage.getPureSaveAccountDetails().getText()).toBe("PureSave Account Details");
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                            ...
                            it('should display the Pure save account details screen and should highlite the message Please enter an amount greater than zero for 0 Rands', function () {
                                expect(transferPage.getPureSaveAccountDetails().getText()).toBe("PureSave Account Details");
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                            ...
                            it('should display the minimum opening balance message as Minimum opening balance: R 50 in puresave account details screen', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                expect(transferPage.getMinimumOpeningBalance().getText()).toContain("Minimum opening balance: R 50");
                            });

                            it('should display the Available balance text and available rand amount in the Pure save account details screen based on the account selected', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                            ...
                            it('should display the available balance based on the account selection in puresave account details', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                expect(transferPage.getRandAmount().getText()).toBe("R 8 756.41");
                                transferPage.FromAccountdata('ACCESSACC - 10-00-530-418-2');
                            ...
                            it('should display the saving and investments screen once we click cancel button pure save account details screen', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                transferPage.Amount('50');
                                transferPage.cancel();
                            ...
                            describe('PW-14295: proceed to Accept offer screen  validations', function () {
                                ...
                                it('should display Your pure save account title and the transfer opening account and amount which is selected', function () {
                                    expect(transferPage.getPureSaveTitleTransfer().getText()).toBe("Your PureSave Account");
                                    ...
                                    expect(acceptOfferPage.getSelectedAmount().getText()).toBe("R 60.00");
                                });
                                ...
                                it('should display Banking Solutions for You screen after clicking the cancel button in accept offer screen', function () {
                                    ...
                                    expect(savingsAndInvestmentsOptionsPage.getTitle().getText()).toBe("Savings and Investments");
                                });

                                it('should display PureSave Account Details screen after clicking the Back button in accept offer screen', function () {
                                    goToAccountApplicationScreens(browser.params.credentials);
                                    transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                    transferPage.Amount('60');
                                    transferPage.proceed();
                                ...
                                describe('PW-14295: proceed to finish screen  validations', function () {
                                    ...
                                    it('Should display the application was unsuccessful message for accountApplicationRejected user', function () {
                                        goToAccountApplicationScreens(browser.params.accountOrigination.accountApplicationRejected);
                                        transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                        transferPage.Amount('60');
                                        transferPage.proceed();
                                        acceptOfferPage.clickCheckBox();
                                        acceptOfferPage.proceed();
                                        expect(savingsFinishPage.getApplicationUnSuccessPage().getText()).toContain('Your application was unsuccessful. Please call 0860 123 000 or visit your nearest branch to find out which savings and investment solutions suit you best');
                                    });
                                });
                            });
                        });
        ...
        describe('PW-14294: Scenario 4: User is SA Citizen with SA ID but Country of Birth is not Populated, Applies for PureSave account and Consents to fraud check', function () {
            it('Should display the basic details page with all required fields uneditable except for Country of Birth which is editable and allow the user to save the country of birth field and clicking cance flow', function () {
                    ...
                    describe('Then proceeds to the income and expenses screen', function () {
                        ...
                        describe('Then proceeds to the consent screen', function () {
                            it('should display the customer\'s marketing consent details', function () {
                                ...
                                consentPage.actions.submit();
                                expect(browser.getLocationAbsUrl()).toContain('/apply/pure-save/transfer');
                            });
                            ...
                            describe('Then proceeds to consent screen', function () {
                                function IncompleteProfile(){
                                    ...
                                    expect(consentPage.contactForSpecialOffers().isEnabled()).toBeFalsy();
                                    consentPage.actions.submit();
                                    expect(savingsAndInvestmentsOptionsPage.getCompleteProfileModal().isDisplayed()).toBeTruthy();
                                }
                                ...
                                it('clicking Yes should redirect to the basic information details page ', function () {
                                        savingsAndInvestmentsOptionsPage.actions.CompleteProfileYesClick();
                                        expect(editBasicInformationPage.infoNotification().isDisplayed()).toBeTruthy();
                                        expect(editBasicInformationPage.infoNotification().getText()).toBe("Please enter all the additional required information to complete your profile");
                                });
                                it('clicking No should hide the Incomplete Profile box', function () {
                                        savingsAndInvestmentsOptionsPage.actions.CompleteProfileNoClick();
                                        expect(savingsAndInvestmentsOptionsPage.getCompleteProfileModal().isDisplayed()).toBeFalsy();
                                });
                            });
                        });
                    });
        ...
        describe('PW-14294: Scenario 5: User is NOT a SA Citizen but has SA ID and Nationality, Country of Birth and Country of Citizenship are not Populated, Applies for PureSave account and Consents to fraud check', function () {
        ...
        describe('PW-14294: Scenario 6: User is has no SA ID document and Nationality, Country of Birth, Country of Citizenship and Permit details are not Populated, Applies for PureSave account and Consents to fraud check', function () {
            it('Should display the basic details page with all required fields uneditable except for Country of Birth which is editable', function () {
    ...
    describe('When a customer that CANNOT apply for pureSave Clicks on apply button for pureSave', function () {
        it('Should refer the customer to a branch', function () {
            goToSavingsAndInvestments(browser.params.accountOrigination.notKYC);
            savingsPrescreeningWidget.actions.apply("pure-save");
            expect(savingsPrescreeningWidget.getKycNonCompliantMessageModal("pure-save").isDisplayed()).toBeTruthy();
            savingsPrescreeningWidget.actions.kycNonCompliantMessageModalClose("pure-save");
            expect(savingsPrescreeningWidget.getKycNonCompliantMessageModal("pure-save").isDisplayed()).toBeFalsy();
            savingsAndInvestmentsOptionsPage.actions.pureSaveDetails();
            savingsPrescreeningWidget.actions.apply("pure-save");
            expect(savingsPrescreeningWidget.getKycNonCompliantMessageModal("pure-save").isDisplayed()).toBeTruthy();
            savingsPrescreeningWidget.actions.kycNonCompliantMessageModalClose("pure-save");
