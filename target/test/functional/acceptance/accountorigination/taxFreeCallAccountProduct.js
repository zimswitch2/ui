...
describe('ACCEPTANCE - taxfree Product', function () {
    'use strict';
    ...
    var taxFreeCallAccountProductPage = require('../../pages/taxFreeCallAccountProductPage.js');
    ...
    var goToAccountApplicationScreens = function (user) {
        ...
        savingsPrescreeningWidget.actions.apply("tax-free-call-account");
        savingsPrescreeningWidget.actions.fraudCheckConsentCheckBoxClick("tax-free-call-account");
        savingsPrescreeningWidget.actions.fraudCheckConsentFormNextClick("tax-free-call-account");
        browser.getLocationAbsUrl().then(function (url) {
            if (url.indexOf('/apply/tax-free-call-account/transfer') < 0) {
    ...
    describe('When a customer that can apply for Tax-Free', function () {
        describe('PW-14271: Scenario 1, 2 and 3: User has all AML fields populated', function () {
            it('Should display the Tax-Free product tile', function () {
                goToSavingsAndInvestments(browser.params.credentials);
                expect(savingsPrescreeningWidget.getFraudCheckModal('tax-free-call-account').isDisplayed()).toBeFalsy();
                expect(savingsAndInvestmentsOptionsPage.getTaxfreeProductContainerHeader()).toEqual('Tax-Free Call Account');
                expect(savingsAndInvestmentsOptionsPage.getTaxfreeProductContainerMessage()).toEqual('Save up to R 30 000 a year -- tax free');
            });

            describe('PW 14272: Then clicks on Details button in the taxFree Product Tile', function () {
                it('Should show the Tax-Free product page', function () {
                    savingsAndInvestmentsOptionsPage.actions.taxFreeCallDetails();
                    expect(taxFreeCallAccountProductPage.getTitle()).toEqual('Apply for Savings Account');
                    expect(taxFreeCallAccountProductPage.baseActions.getCurrentUrl()).toContain('/apply/tax-free-call-account');
                    expect(taxFreeCallAccountProductPage.getProductTitle()).toContain('Tax-Free Call Account');
                    expect(taxFreeCallAccountProductPage.getPureSaveProductDescription()).toEqual('Invest up to R 30 000 a year and a maximum of R 500 000 in your lifetime - and get tax-free returns on your contributions');
                    expect(taxFreeCallAccountProductPage.getContentHeaders()).toContain('Highlights');
                    expect(taxFreeCallAccountProductPage.getContentHeaders()).toContain('How to qualify');
                    expect(taxFreeCallAccountProductPage.getContentHeadersPanelTwo()).toContain('What it costs');
                    expect(taxFreeCallAccountProductPage.getContentHeadersPanelTwo()).toContain('What you\'ll need');
                    expect(taxFreeCallAccountProductPage.getPricingGuide()).toEqual('Tax-Free Call Account pricing (PDF)');
                    expect(taxFreeCallAccountProductPage.getTitle()).toEqual('Apply for Savings Account');
                    expect(savingsPrescreeningWidget.applyButtonVisible("tax-free-call-account").isDisplayed()).toBeTruthy();
                    expect(taxFreeCallAccountProductPage.backtoSavingInvestment().isDisplayed()).toBeTruthy();
                });

                describe('Then clicks on the apply button', function () {
                    it('should display the fraud check consent form', function () {
                        savingsPrescreeningWidget.actions.apply("tax-free-call-account");
                        expect(savingsPrescreeningWidget.getFraudCheckModal("tax-free-call-account").isDisplayed()).toBeTruthy();
                        expect(savingsPrescreeningWidget.getFraudCheckModalNextButton("tax-free-call-account").isEnabled()).toBeFalsy();
                    });

                    describe('Then clicks on fraud check consent checkbox', function () {
                        it('should enable the next button', function () {
                            savingsPrescreeningWidget.actions.fraudCheckConsentCheckBoxClick("tax-free-call-account");
                            expect(savingsPrescreeningWidget.getFraudCheckModalNextButton("tax-free-call-account").isEnabled()).toBeTruthy();
                        });

                        describe('Then clicks on next button', function () {
                            it('Should create the taxFree flow and direct the user to the account application transfer page', function () {
                                savingsPrescreeningWidget.actions.fraudCheckConsentFormNextClick("tax-free-call-account");
                                expect(taxFreeCallAccountProductPage.baseActions.getCurrentUrl()).toContain('/apply/tax-free-call-account/transfer');
                    ...
                    describe('Then clicks on cancel button', function () {
                        it('should close the fraud check consent form and show popup asking customer to agree to fraud check', function () {
                            goToSavingsAndInvestments(browser.params.credentials);
                            savingsPrescreeningWidget.actions.apply("tax-free-call-account");
                            savingsPrescreeningWidget.actions.fraudCheckConsentFormCancelClick("tax-free-call-account");
                            expect(savingsPrescreeningWidget.getFraudCheckModal('tax-free-call-account').isDisplayed()).toBeFalsy();
                        });
                    });
                });
            ...
            describe('Then Clicks on apply button for taxFree in the product tile', function () {
                it('should display the fraud check consent form', function () {
                    goToSavingsAndInvestments(browser.params.credentials);
                    savingsPrescreeningWidget.actions.apply("tax-free-call-account");
                    expect(savingsPrescreeningWidget.getFraudCheckModal('tax-free-call-account').isDisplayed()).toBeTruthy();
                ...
                describe('Then clicks on fraud check consent checkbox', function () {
                    ...
                    describe('Then clicks on next button', function () {
                        it('Should create the taxFree flow and direct the user to the account application transfer page', function () {
                            savingsPrescreeningWidget.actions.fraudCheckConsentFormNextClick("tax-free-call-account");
                            expect(currentAccountDetailsPage.baseActions.getCurrentUrl()).toContain('/apply/tax-free-call-account/transfer');
                            expect(currentAccountDetailsPage.baseActions.flow.numberOfSteps()).toEqual(4);
                            expect(currentAccountDetailsPage.baseActions.flow.currentStep()).toEqual('Transfer');
                            currentAccountDetailsPage.baseActions.flow.steps().then(function (flowSteps) {
                                expect(flowSteps[1].getText()).toEqual('Transfer');
                        ...
                        describe('PW-14295: proceed to Account details screen validations', function () {
                            it('should display the Taxfree account details screen and should through the amount exceeds error for low balance account', function () {
                                expect(transferPage.getTaxFreeAccountDetails().getText()).toBe("Tax-Free Call Account Details");
                                expect(transferPage.getAmountDefaultValue()).toBe("250");
                                transferPage.FromAccountdata('ACCESSACC - 10-00-530-418-2');
                                transferPage.Amount('250');
                                expect(transferPage.getAmountExceedsErrorMessage().getText()).toContain("The amount exceeds your available balance");
                            });

                            it('should display the tax tax-free account details screen and should through the minimum amount message for less 50 rands', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                transferPage.Amount('12');
                                expect(transferPage.getEnterAnAmountAtleastMessage().getText()).toContain("Enter an amount of at least R250");
                            });

                            it('should display the tax tax-free account details screen and should highlite the message Please enter an amount greater than zero for 0 Rands', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                            ...
                            it('should display the minimum opening balance message as Minimum opening balance: R 50.00 in taxFree account details screen', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                expect(transferPage.getMinimumOpeningBalance().getText()).toContain("Minimum opening balance: R 250.00");
                            });

                            it('should display the Available balance text and available rand amount in taxfree account details screen based on the account selected', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                            ...
                            it('should display the available balance based on the account selection in taxFree account details', function () {
                                transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                expect(transferPage.getRandAmount().getText()).toBe("R 8 756.41");
                                transferPage.FromAccountdata('ACCESSACC - 10-00-530-418-2');
                            ...
                            describe('PW-14295: proceed to Accept offer screen  validations', function () {
                                ...
                                it('should display Your pure save account title and the transfer opening account and amount which is selected', function () {
                                    ...
                                    expect(acceptOfferPage.getSelectedAmount().getText()).toBe("R 250.00");
                                });
                                ...
                                it('should display taxFree Account Details screen after clicking the Back button in accept offer screen', function () {
                                    goToAccountApplicationScreens(browser.params.credentials);
                                    transferPage.FromAccountdata('ACCESSACC - 10-00-035-814-0');
                                    transferPage.Amount('250');
                                    transferPage.proceed();
                                    acceptOfferPage.clickBackBtn();
                                    expect(transferPage.getPureSaveAccountDetails().getText()).toBe("Tax-Free Call Account Details");
                                });
                                ...
                                describe('PW-14295: proceed to finish screen  validations', function () {
                                    it('should display the Your application was successful and summary in finish screen', function () {
                                        ...
                                        expect(savingsFinishPage.getAccountType().getText()).toBe("Tax-Free Call");
                ...
                describe('click on cancel button', function () {
                    it('should close the fraud check consent form and display a message to the user', function () {
                        ...
                        savingsPrescreeningWidget.actions.fraudCheckConsentFormCancelClick("tax-free-call-account");
                        expect(savingsPrescreeningWidget.getFraudCheckModal('tax-free-call-account').isDisplayed()).toBeFalsy();
                        expect(currentAccountDetailsPage.baseActions.getCurrentUrl()).toContain('/apply/savings-and-investments');
                    });
                });
            });
        });

        describe('PW-14294: Scenario 4: User is SA Citizen with SA ID but Country of Birth is not Populated, Applies for taxFree account and Consents to fraud check', function () {
                    ...
                    describe('Then proceeds to the income and expenses screen', function () {
                        ...
                        describe('Then proceeds to the consent screen', function () {
                            it('should display the customer\'s marketing consent details', function () {
                                ...
                                consentPage.actions.submit();
                                expect(browser.getLocationAbsUrl()).toContain('/apply/tax-free-call-account/transfer');
                            });
        ...
        describe('PW-14294: Scenario 5: User is NOT a SA Citizen but has SA ID and Nationality, Country of Birth and Country of Citizenship are not Populated, Applies for taxFree account and Consents to fraud check', function () {
    ...
    describe('When a customer that CANNOT apply for Taxfree Clicks on apply button for taxFree', function () {
        it('Should refer the customer to a branch', function () {
            goToSavingsAndInvestments(browser.params.accountOrigination.notKYC);
            savingsPrescreeningWidget.actions.apply("tax-free-call-account");
            expect(savingsPrescreeningWidget.getKycNonCompliantMessageModal("tax-free-call-account").isDisplayed()).toBeTruthy();
            savingsPrescreeningWidget.actions.kycNonCompliantMessageModalClose("tax-free-call-account");
            expect(savingsPrescreeningWidget.getKycNonCompliantMessageModal("tax-free-call-account").isDisplayed()).toBeFalsy();
            savingsAndInvestmentsOptionsPage.actions.taxFreeCallDetails();
            savingsPrescreeningWidget.actions.apply("tax-free-call-account");
            expect(savingsPrescreeningWidget.getKycNonCompliantMessageModal("tax-free-call-account").isDisplayed()).toBeTruthy();
            savingsPrescreeningWidget.actions.kycNonCompliantMessageModalClose("tax-free-call-account");
